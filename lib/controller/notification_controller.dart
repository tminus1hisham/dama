...
class NotificationController extends GetxController {
  ...
  Future<void> _loadLocallyReadIds() async {
    try {
      ...
      debugPrint('Loaded ${_locallyReadIds.length} locally read notification IDs');
    } catch (e) {
  ...
  Future<void> _saveLocallyReadIds() async {
    try {
      final prefs = await SharedPreferences.getInstance();
      await prefs.setStringList(_readNotificationsKey, _locallyReadIds.toList());
      debugPrint('Saved ${_locallyReadIds.length} locally read notification IDs');
    } catch (e) {
  ...
  Future<void> fetchnotifications() async {
    ...
    try {
      ...
      int mergedCount = 0;
      final mergedNotifications = fetchedNotifications.map((notification) {
      ...
      debugPrint('Merged $mergedCount notifications as read from local storage');
  ...
  Future<bool> markAllAsRead() async {
    try {
      ...
      final unreadNotifications = notificationList.where((n) => !n.read && n.id.isNotEmpty).toList();
      ...
      debugPrint('Marking ${unreadNotifications.length} notifications as read locally...');
      ...
      debugPrint('Successfully marked ${unreadNotifications.length} notifications as read locally');
      return true;
    } catch (e) {
